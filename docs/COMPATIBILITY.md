...
| Image generation | Supported for remote API providers | Comfy RunPod, Together, Pollinations, Stability, Hugging Face, Chutes, ElectronHub, NanoGPT, BFL, FAL, xAI, AIMLAPI, Z.AI and Workers AI. Local Automatic1111/SD.Next/sd.cpp/DrawThings/Comfy hosts are not offered. Binary output is streamed through bounded R2 hand-off objects and encoded by the browser. |
...
| Vector Storage API | Compatibility mode | Same-origin `/api/vector/*` with bounded D1 keyword retrieval; no embedding model download, browser inference, or local vector server. |
