...
async function uploadedFile(request: Request, env: Env): Promise<{ file: File; fields: Record<string, string> }> {
    ...
    if (file.size > maxUploadBytes(env)) throw new HttpError(413, 'Upload exceeds configured limit');
    return { file, fields };
}
...
async function serveCoreAssetFallback(env: Env, kind: string, name: string, request: Request): Promise<Response> {
    if (await findObject(env, kind, name)) return serveObject(env, kind, name, request);
    const fallback = await env.ASSETS.fetch(request);
    if (!fallback.ok) throw new HttpError(404, 'File not found');
    return fallback;
}
...
export function registerMediaRoutes(router: Router): void {
    ...
    router.on('GET', '/backgrounds/:name', ({ request, env, params }) => serveCoreAssetFallback(env, 'background', cleanFileName(params.name), request));
    router.on('HEAD', '/backgrounds/:name', ({ request, env, params }) => serveCoreAssetFallback(env, 'background', cleanFileName(params.name), request));
    ...
    const serveAvatar = ({ request, env, params }: { request: Request; env: Env; params: Readonly<Record<string, string>> }) => serveCoreAssetFallback(env, 'user-avatar', cleanFileName(params.name), request);
    ...
    router.on('POST', '/api/images/upload', async ({ request, env }) => {
        ...
        throw new HttpError(415, 'Base64 image uploads are disabled; upload multipart binary from the browser');
    });
    ...
    router.on('POST', '/api/files/upload', async ({ request, env }) => {
        const { file } = await uploadedFile(request, env);
        const name = cleanFileName(file.name, 'name');
        await putObject(env, 'user-file', name, file.stream(), {
            mimeType: file.type || contentTypeFor(name),
            byteLength: file.size,
        });
