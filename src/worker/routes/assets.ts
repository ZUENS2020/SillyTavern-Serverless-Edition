...
export function registerAssetRoutes(router: Router): void {
    ...
    router.on('POST', '/api/assets/download', async () => {
        throw new HttpError(410, 'Remote asset proxying is disabled; download in the browser and upload the file directly');
    });
    ...
    router.on('GET', '/assets/*', async ({ request, env, params }) => {
        ...
        if (await findObject(env, 'asset', name)) return serveObject(env, 'asset', name, request);
        const fallback = await env.ASSETS.fetch(request);
        if (!fallback.ok) throw new HttpError(404, 'Asset not found');
        return fallback;
    });
