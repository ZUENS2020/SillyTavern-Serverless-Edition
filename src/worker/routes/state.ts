...
import { deleteState, getState, listState, putState, SETTINGS_VIEW_CACHE_KEY } from '../storage/state';
...
async function settingsResponse(env: Env): Promise<Response> {
    const cached = await env.CACHE.get(SETTINGS_VIEW_CACHE_KEY);
    if (cached) {
        return new Response(cached, {
            headers: { 'content-type': 'application/json; charset=utf-8', 'cache-control': 'private, no-store' },
        });
    }
    const [settings, customState] = await Promise.all([
        getState<JsonObject>(env, 'settings', 'current'),
        env.DB.prepare(`
            SELECT EXISTS(SELECT 1 FROM presets LIMIT 1)
                OR EXISTS(SELECT 1 FROM themes LIMIT 1)
                OR EXISTS(SELECT 1 FROM world_books LIMIT 1) AS present
        `).first<{ present: number }>(),
    ]);
    let encoded: string;
    if (!customState?.present) {
        const suffixAt = DEFAULT_SETTINGS_PAYLOAD.indexOf(',"koboldai_settings":');
        if (suffixAt < 0) throw new Error('Generated settings payload is invalid');
        const settingsText = settings ? JSON.stringify(settings.value) : DEFAULT_SETTINGS_TEXT;
        encoded = `{"settings":${JSON.stringify(settingsText)}${DEFAULT_SETTINGS_PAYLOAD.slice(suffixAt)}`;
    } else {
        encoded = JSON.stringify(await settingsPayload(env));
    }
    await env.CACHE.put(SETTINGS_VIEW_CACHE_KEY, encoded, { expirationTtl: 3_600 });
...
async function replaceGroupMembers(env: Env, groupId: string, group: JsonObject): Promise<void> {
    const members = Array.isArray(group.members)
        ? group.members.filter((value): value is string => typeof value === 'string').slice(0, 100)
        : [];
    const disabled = new Set(Array.isArray(group.disabled_members)
        ? group.disabled_members.filter((value): value is string => typeof value === 'string')
        : []);
    const statements = [env.DB.prepare('DELETE FROM group_members WHERE group_id = ?').bind(groupId)];
    for (const [position, avatar] of members.entries()) {
        statements.push(env.DB.prepare(`
            INSERT INTO group_members(group_id, character_avatar, position, disabled) VALUES (?, ?, ?, ?)
        `).bind(groupId, avatar, position, disabled.has(avatar) ? 1 : 0));
    }
...
export function registerStateRoutes(router: Router): void {
    ...
    router.on('POST', '/api/settings/get', ({ env }) => settingsResponse(env));
    ...
    router.on('POST', '/api/groups/create', async ({ request, env }) => {
        ...
        await replaceGroupMembers(env, id, group);
        return json(group);
    });
