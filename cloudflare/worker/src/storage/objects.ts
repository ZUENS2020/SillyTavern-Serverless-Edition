...
export function objectKey(kind: string, name: string): string {
    return `single/${encodeURIComponent(kind)}/${encodeURIComponent(name)}`;
}

export async function findObject(env: Env, kind: string, name: string): Promise<StoredObject | null> {
    const row = await env.DB.prepare(`
        SELECT id, kind, name, r2_key, mime_type, byte_length, etag, metadata, created_at, updated_at
...
export async function putObject(
    ...
    try {
        await env.DB.prepare(`
            INSERT INTO objects(id, kind, name, r2_key, mime_type, byte_length, etag, metadata, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
